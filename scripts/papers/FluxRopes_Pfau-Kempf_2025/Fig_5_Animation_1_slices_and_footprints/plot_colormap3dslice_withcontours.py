...
def fmt(x, pos):
    ...
    return signchar+'{}'.format(abs(float(a)))+r'*'+'10^{}'.format(int(b))
...
def plot_colormap3dslice(filename=None,
                  ...
                  streamlines=None, streamlinedensityx=1, streamlinedensityy=1, streamlinecolor='white', streamlinethick=1.0,
                  axes=None, cbaxes=None, cbaxhint=None,
                  normal='y', cutpoint=0., cutpointre=None,
                  contourvariable=None, contouroperator="pass", contourvalues=[1.0],contourcolors=["black"],contourlinewidths=[1.0],
                  fluxropelevel=0., fluxropemaxcurvatureradius=1e10, fluxropemarker="$\mathrm{F}$", fluxropemarkerfacecolor="black", fluxropemarkeredgecolor="None",fluxropemarkersize=10, fluxropelinewidths=1,
                  xmarker="X", xmarkerfacecolor="black", xmarkeredgecolor="None", xmarkersize=10, xlinewidths=1,
                  omarker="O", omarkerfacecolor="black", omarkeredgecolor="None", omarkersize=10, olinewidths=1,
                  ):
    ...
    :kwarg outputdir:   path to directory where output files are created (default: $HOME/Plots/)
    ...
    if operator is not None:
        ...
        if operator!='x' and operator!='y' and operator!='z' and operator!='magnitude' and not operator.isdigit():
            ...
            operator=None
            operatorstr=''
        if operator=='x' or operator=='y' or operator=='z' or operator=='magnitude':
            # For components, always use linear scale, unless symlog is set
            operatorstr='_'+operator
            if symlog is None and operator!='magnitude':
                lin=1
        # index a vector
        if operator.isdigit():
            operator = str(operator)
    ...
    # Read vg_fluxrope
    ############################################
    if fluxropelevel != 0 and timeval > 1000.1:
        fluxropemap = f.read_variable("vg_fluxrope")
        curvaturemap = f.read_variable("vg_curvature", operator="magnitude")
        fluxropemap = fluxropemap[indexids] # sort
        fluxropemap = fluxropemap[indexlist] # find required cells
        curvaturemap = curvaturemap[indexids] # sort
        curvaturemap = curvaturemap[indexlist] # find required cells

        fluxropecellids = cellids[indexlist[np.argwhere(np.logical_and(np.logical_and(fluxropemap>0., fluxropemap<fluxropelevel), np.reciprocal(curvaturemap+1e-23) < fluxropemaxcurvatureradius))]].squeeze()
        fluxropecoords = f.get_cell_coordinates(fluxropecellids)

    ############################################
    # Read vg XO
    ############################################
    if vlsvxo is not None and timeval > 1000.1:
        xomap = vlsvxo.read_variable("vg_lmn_neutral_line_distance")
        xomap = xomap[indexids] # sort
        xomap = xomap[indexlist] # find required cells
        dBNdLmap = vlsvxo.read_variable("LMN_magnetopause/vg_jacobian", operator=6)
        dBNdLmap = dBNdLmap[indexids] # sort
        dBNdLmap = dBNdLmap[indexlist] # find required cells
        xcellids = cellids[indexlist[np.argwhere(np.logical_and(xomap<0.2, dBNdLmap>0))]].squeeze()
        ocellids = cellids[indexlist[np.argwhere(np.logical_and(xomap<0.2, dBNdLmap<0))]].squeeze()
        xcoords = np.ma.masked_array(f.get_cell_coordinates(xcellids))
        ocoords = np.ma.masked_array(f.get_cell_coordinates(ocellids))
        xcoords[:,0] = np.ma.masked_where(np.linalg.norm(xcoords, axis=-1) < 30e6, xcoords[:,0])
        ocoords[:,0] = np.ma.masked_where(np.linalg.norm(ocoords, axis=-1) < 30e6, ocoords[:,0])
        xcoords=np.ma.mask_rows(xcoords)
        ocoords=np.ma.mask_rows(ocoords)
    ...
    # Make vmaxuse and vminuse and lin available for formatter functions
    ...
    if lin is None:
        ...
        ticks = np.linspace(vminuse,vmaxuse,num=5)
    ...
    if noxlabels is None:
        ax1.set_xlabel(axislabels[0]+' ['+axisunitstr+']',fontsize=fontsize,weight='black')
    ...
    if noylabels is None:
        ax1.set_ylabel(axislabels[1]+' ['+axisunitstr+']',fontsize=fontsize,weight='black')
    ...
    if vectors != None:
        ...
        # Read vg betastar
        ############################################
        if vectors == "vg_poynting":
            betastarmap = f.read_variable("vg_beta_star")
            betastarmap = betastarmap[indexids] # sort
            betastarmap = betastarmap[indexlist] # find required cells
            betastarmap = ids3d.idmesh3d(idlist, betastarmap, reflevel, xsize, ysize, zsize, xyz, None)
            if np.ma.is_masked(maskgrid):
                betastarmap = betastarmap[MaskX[0]:MaskX[-1]+1,:]
                betastarmap = betastarmap[:,MaskY[0]:MaskY[-1]+1]
            if np.ma.is_masked(rhomap):
                betastarmap = np.ma.array(betastarmap, mask=XYmask)
        ...
        lengths=np.ma.masked_outside(lengths, 1e-6, 1e-3)
        lengths=np.ma.masked_where(np.ma.getmask(np.ma.masked_outside(betastarmap, 0.4, 0.6)), lengths)
        # YPK THIS WILL NOW BE USED FOR ACTUAL LENGTH SCALING
        colors = np.ma.log10(lengths) + 6
        ...
        # YPK misuse the log10-scaled "colors" above to scale the vector lengths
        U *= C
        V *= C
        ...
        if cbaxhint=="vectmap":
            fig1 = ax1.quiver(X,Y,U,V,C, cmap=vectorcolormap, units='dots', scale=0.05/vectorsize, headlength=4, headwidth=4,
                   headaxislength=2, scale_units='dots', pivot='tail', width=2)
        else:
    ...
    if streamlines is not None:
        ...
        seeds = np.array(np.meshgrid(np.arange(boxcoords[0], boxcoords[1], streamlinedensityx), np.arange(boxcoords[2], boxcoords[3], streamlinedensityy))).T.reshape((-1,2))
        ax1.streamplot(XmeshCentres,YmeshCentres,U,V,linewidth=0.5*streamlinethick, color=streamlinecolor, minlength=0, maxlength=0.07, start_points=seeds, arrowsize=0, density=100)
    ...
    if fluxropelevel != 0 and timeval > 1000.1:
        ...
        if normal == [0,0,1]:
            ...
            indexy = 1
        try:
            ax1.scatter(fluxropecoords[:,indexx]/axisunit, fluxropecoords[:,indexy]/axisunit, marker=fluxropemarker, s=fluxropemarkersize, linewidths=fluxropelinewidths, edgecolor=fluxropemarkeredgecolor, facecolor=fluxropemarkerfacecolor)
        except:
            print("No points in fluxrope scatter data")
    ...
    if vlsvxo is not None and timeval > 1000.1:
        ...
        if normal == [0,0,1]:
            ...
            indexy = 1
        try:
            ax1.scatter(xcoords[:,indexx]/axisunit, xcoords[:,indexy]/axisunit, marker=xmarker, s=xmarkersize, linewidths=xlinewidths, edgecolor=xmarkeredgecolor, facecolor=xmarkerfacecolor)
        except:
            print("No points in X scatter data")
#        try:
        ax1.scatter(ocoords[:,indexx]/axisunit, ocoords[:,indexy]/axisunit, marker=omarker, s=omarkersize, linewidths=olinewidths, edgecolor=omarkeredgecolor, facecolor=omarkerfacecolor)
#        except:
#            print("No points in O scatter data")


    if contourvariable!=None:
        contourcolour = 'white'
        contourmap = f.read_variable(contourvariable, operator=contouroperator)
        contourmap = contourmap[indexids] # sort
        contourmap = contourmap[indexlist] # find required cells
        contourmap = ids3d.idmesh3d(idlist, contourmap, reflevel, xsize, ysize, zsize, xyz, None)
        if np.ma.is_masked(maskgrid):
            contourmap = contourmap[MaskX[0]:MaskX[-1]+1,:]
            contourmap = contourmap[:,MaskY[0]:MaskY[-1]+1]
        if np.ma.is_masked(rhomap):
            contourmap = np.ma.array(contourmap, mask=XYmask)
        contourcont = ax1.contour(XmeshCentres,YmeshCentres,contourmap,contourvalues,colors=contourcolors,
                                  linestyles='solid',linewidths=contourlinewidths,zorder=2)
    ...
    if nocb is None:
        ...
        if cbaxes is None:
            ...
            cb_title = cax.set_title(cb_title_use,fontsize=fontsize,fontweight='bold', horizontalalignment=horalign, x=0.0, y = 1.05)
            # YPK replicate here
#            cb_title.set_position((0.,1.+0.1*scale)) # avoids having colourbar title too low when fontsize is increased
        ...
        if lin is None and symlog is None:
            ...
            for label in cb.ax.yaxis.get_ticklabels():
                if usesci is True:
                    firstdigit = label.get_text().replace('$','')[1]
                else:
                    if float(label.get_text().replace('$','')) == 0.0:
                        firstdigit = '0'
                    else:
