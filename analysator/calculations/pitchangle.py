...
def pitch_angles( vlsvReader,
   ...
   ''' Calculates the pitch angle distribution for a given cell
      ...
      :kwarg filename:          path to .vlsv file to use for input.
      ...
      :kwarg nbins:             How many bins to use for the distribution
      :kwarg cosine:            True if returning the pitch angles as a cosine(alpha) plot [-1,1].
                              If false, returns as pitch angle in degrees [0,180].

      :kwarg plasmaframe:       True if the user wants to get the pitch angle distribution
      ...
      :kwarg vcut:              Set to True to ignore velocity cells below 2x the thermal speed.
                              If set to a number, will use that velocity in m/s instead.
      :kwarg vcutmax:           Set to True to ignore velocity cells above 2x the thermal speed.
                              If set to a number, will use that velocity in m/s instead.

      :kwarg outputdir:         Optional (recommended) method to save results to a file in the given directory.
                              If directory does not exist, it will be created. Filenames within directory are
                              generated automatically.
      :kwarg outputfile:        Provide exact output file name (including complete path)

      :kwarg pop:               Active population, defaults to proton (avgs)
