...
parser.add_argument(
    "--generate", "-g", help="Generate hashdump into given path", const=str, nargs="?"
)
parser.add_argument(
    "--compare", "-c", help="Compare against given hashdump", const=str, nargs="?"
)
...
class Tester:
    ...
    def loadFromFile(self, dumpname, compare_path):
        outdict = {}
        ...
        with open(path_to_file, "r") as file:
            for line in file:
                ...
                if ("Function:" in line and "[" not in line):  # bit stupid but works for now
                    ...
                    outdict[filename][function][listHashInfo[0]] = [
                        listHashInfo[1],
                        listHashInfo[2],
                    ]
                else:
    ...
    def interpolationtest2d(self, varname):
        ...
        coords = np.hstack(
            (
                np.reshape(X, (ncoords))[:, np.newaxis],
    ...
    def interpolationtest3d(self):
        RE = 6371e3
        coords = [
            [5 * RE, RE, 0.5 * RE],
            np.array([[10 * RE, RE, 0.5 * RE], [5 * RE, RE, 0.1 * RE]]),
            np.array([[5 * RE, RE, 0.5 * RE], [8 * RE, RE, 0.1 * RE]]),
        ]
        for i, coord in enumerate(coords):
            self.hash(
                "read_interpolated_variable",
                ["proton/vg_rho", coord],
                argkey_name=f"proton/vg_rho_{i}",
                flatten=False,
            )
            self.hash(
                "read_interpolated_variable",
                ["proton/vg_v", coord],
                argkey_name=f"proton/vg_v_{i}",
                flatten=False,
            )
            self.hash(
                "read_interpolated_variable",
                ["proton/vg_ptensor", coord],
                argkey_name=f"proton/vg_ptensor_{i}",
                flatten=False,
            )
    ...
    def compareAgainstRef(self, hashdump, compare_path, hashdict=None):
        ...
        retval = True  # We want the full diagnostics of the compare, so we need a return value instead of raising errors
        if not hashdict:
...
if __name__ == "__main__":
    ...
    for file in files:
        ...
        # Handle input variables and filter them for the calls used later
        variables_to_test = [
            "CellID",
            "vg_rhom",
            "vg_v",
        ...
        if "3D" in ciTester.filename:
            ...
            ciTester.hash(
                pt.calculations.cut3d,
                {
                    ...
                    "zmin": zmin,
                    "zmax": zmax,
                    "variable": "vg_connection",
                },
                novlsv=True,
                argkey_name="vg_connection",
                flatten=False,
            )
    ...
    if generate_path:
        if ciTester.hashes_dict_python:
            ciTester.dumpIntoFile(
                ciTester.hashes_dict_python, "hashdump_python.txt", generate_path
            )
        ...
        if ciTester.hashes_dict_rust:
            ciTester.dumpIntoFile(
                ciTester.hashes_dict_rust, "hashdump_rust.txt", generate_path
            )
