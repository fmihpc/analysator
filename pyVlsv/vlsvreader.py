...
class VlsvReader(object):
   ...
   def __init__(self, file_name, fsGridDecomposition=None):
      ...
      try:
         ...
         logger.info("File not found: ", self.file_name)
         raise e
      ...
      for child in self.__xml_root:
          if child.tag == "BLOCKIDS":
              ...
              if not os.getenv('PTNONINTERACTIVE'):
                 logger.info("Found population " + popname)
   ...
   def __read_blocks(self, cellid, pop="proton"):
      ...
      if self.use_dict_for_blocks: #deprecated version
         ...
         try:
            ...
            logger.info("Cell does not have velocity distribution")
            return []
...
#               offset_avgs += blocks_per_cell[i]*vector_size*element_size
         ...
         if (pop == 'avgs' or ("name" in child.attrib) and (child.attrib["name"] == pop)) and (child.tag == "BLOCKIDS"):
            ...
            if datatype == "uint" and element_size == 4:
               ...
               logger.info("Error! Bad block id data!")
               logger.info("Data type: " + datatype + ", element size: " + str(element_size))
               return
      ...
      logger.info("data_avgs = " + str(data_avgs) + ", data_block_ids = " + str(data_block_ids))
      if len(data_avgs) != len(data_block_ids):
         logger.info("BAD DATA SIZES")
   ...
   def __set_cell_offset_and_blocks(self, pop="proton"):
      ...
      '''
      logger.info("Getting offsets for population " + pop)
   ...
   def list(self, parameter=True, variable=True, mesh=False, datareducer=False, operator=False, other=False):
      ...
      '''
      if parameter:
         logger.info("tag = PARAMETER")
         for child in self.__xml_root:
            if child.tag == "PARAMETER" and "name" in child.attrib:
               logger.info("   ", child.attrib["name"])
      if variable:
         logger.info("tag = VARIABLE")
         for child in self.__xml_root:
            if child.tag == "VARIABLE" and "name" in child.attrib:
               logger.info("   ", child.attrib["name"])
      if mesh:
         logger.info("tag = MESH")
         for child in self.__xml_root:
            if child.tag == "MESH" and "name" in child.attrib:
               logger.info("   ", child.attrib["name"])
      if datareducer:
         logger.info("Datareducers:")
         for name in datareducers:
            logger.info("   ",name, " based on ", datareducers[name].variables)
      if operator:
         logger.info("Data operators:")
      ...
      if other:
         ...
         for child in self.__xml_root:
            if child.tag != "PARAMETER" and child.tag != "VARIABLE" and child.tag != "MESH":
               logger.info("    tag = ", child.tag, " mesh = ", child.attrib["mesh"])
   ...
   def print_version(self):
      ...
      logger.info("File ",self.file_name," contains no version information")
      return False
   ...
   def print_config(self):
      ...
      if config_string is not None:
         ...
         #if we end up here the file does not contain any config info
         logger.info("File ",self.file_name," contains no config information")
         return False
   ...
   def read_attribute(self, name="", mesh="", attribute="", tag=""):
      ...
      '''
      if tag == "" and name == "":
         logger.info("Bad (empty) arguments at VlsvReader.read")
         raise ValueError()
   ...
   def read(self, name="", tag="", mesh="", operator="pass", cellids=-1):
      ...
      for child in self.__xml_root:
         ...
         if child.tag == tag:
            ...
            if vector_size == 1 and operator=="magnitude":
               logger.info("Data variable with vector size 1: Changed magnitude operation to absolute")
               operator="absolute"
      ...
      if name in reducer_reg:
         ...
         if reducer.vector_size == 1 and operator=="magnitude":
            logger.info("Data reducer with vector size 1: Changed magnitude operation to absolute")
            operator="absolute"
         ...
         if reducer.useVspace and not reducer.useReader:
            ...
            for singlecellid in actualcellids:
               ...
               index+=1
               logger.info(index,"/",len(actualcellids))
            
            if reducer.useReader:
               logger.info("Combined useVspace and useReader reducers not implemented!")
      ...
      if 'pop/'+varname in reducer_multipop:
         ...
         if reducer.useVspace:
            logger.info("Error: useVspace flag is not implemented for multipop datareducers!") 
            return
   ...
   def read_metadata(self, name="", tag="", mesh=""):
      ...
      if tag == "" and name == "":
         logger.info("Bad arguments at read")
      ...
      if name!="":
         logger.info("Error: variable "+name+"/"+tag+"/"+mesh+" not found in .vlsv file!" )
      fptr.close()
      return -1
   ...
   def read_interpolated_ionosphere_variable(self, name, coordinates, operator="pass"):
      ...
      # At this stage, this function has not yet been implemented -- logger.info a warning and exit
      logger.info('Interpolation of ionosphere variables has not yet been implemented; exiting.')
      return -1
   ...
   def read_fsgrid_variable(self, name, operator="pass"):
       ...
       if self.__fsGridDecomposition is None:
         ...
         if self.__fsGridDecomposition is not None:
            logger.info("Found FsGrid decomposition from vlsv file: ", self.__fsGridDecomposition)
         else:
            logger.info("Did not find FsGrid decomposition from vlsv file.")
       
       # If decomposition is None even after reading, we need to calculate it:
       if self.__fsGridDecomposition is None:
          logger.info("Calculating fsGrid decomposition from the file")
          self.__fsGridDecomposition = fsDecompositionFromGlobalIds(self)
          logger.info("Computed FsGrid decomposition to be: ", self.__fsGridDecomposition)
       else:
   ...
   def read_fg_variable_as_volumetric(self, name, centering=None, operator="pass"):
      ...
      if centering is None:
         try:
            ...
            logger.info("A variable ("+name+") with unknown centering! Aborting.")
            return False
      ...
      if fgdata.shape[-1] == 3:
         if centering=="face":
            ...
            logger.info("Unknown centering ('" +centering+ "')! Aborting.")
            return False
      else:
         logger.info("A scalar variable! I don't know what to do with this! Aborting.")
         return False
      return celldata
   ...
   def read_variable(self, name, cellids=-1,operator="pass"):
      ...
      if (self.check_variable(name) and (name.lower()[0:3]=="fg_")):
         if not cellids == -1:
            logger.info("Warning, CellID requests not supported for FSgrid variables! Aborting.")
            return False
      ...
      if name.lower()[0:3]=="ig_":
         if not cellids == -1:
            logger.info("Warning, CellID requests not supported for ionosphere variables! Aborting.")
            return False
   ...
   def get_amr_level(self,cellid):
      ...
      while np.any(cellids > 0):
         ...
         if(iters > self.get_max_refinement_level()+1):
            logger.info("Can't have that large refinements. Something broke.")
            break
   ...
   def get_cellid_with_vdf(self, coords, pop = 'proton'):
      ...
      if N_w_vdf==0:
         logger.info("Error: No velocity distributions found!")
         sys.exit()
      ...
      '''
      # Loop approach:
      logger.info('Not enough memory to broadcast arrays! Using a loop instead...')
   ...
   def read_velocity_cells(self, cellid, pop="proton"):
      ...
      logger.info "Velocity cell value at velocity cell id " + str(random_velocity_cell_id) + ": " + str(velocity_cell_map[random_velocity_cell_id])
      ...
      logger.info "Velocity cell value at velocity cell id " + str(random_velocity_cell_id) + "and coordinates " + str(random_velocity_cell_coordinates) + ": " + str(velocity_cell_map[random_velocity_cell_id])
   ...
   def get_fsgrid_indices(self, coords):
      ...
      if (ri < 0).any() or (ri>sz-1).any():
         logger.info("get_fsgrid_indices: Resulting index out of bounds, returning None")
         return None
      return tuple(ri)
   ...
   def get_ionosphere_mesh_size(self):
      ...
      try:
         ...
         logger.info("Error: Failed to read ionosphere mesh size. Are you reading from a file without ionosphere?")
         return [0,0]
   ...
   def get_ionosphere_node_coords(self):
      ...
      try:
         ...
         logger.info("Error: Failed to read ionosphere mesh coordinates. Are you reading from a file without ionosphere?")
         return []
   ...
   def get_ionosphere_element_corners(self):
      ...
      try:
         ...
         logger.info("Error: Failed to read ionosphere mesh elements. Are you reading from a file without ionosphere?")
         return []
   ...
   def optimize_clear_fileindex_for_cellid_blocks(self):
      ''' Clears a private variable containing number of blocks and offsets for particular cell ids
         ...
         .. code-block: python
             ...
             # Go through vlsv readers and logger.info info:
             for vlsvReader in vlsvReaders:
                # Print something from the file on the screen
                logger.info vlsvReader.read_blocks( cellid= 5021 ) # Stores info into a private variable
   ...
   def optimize_clear_fileindex_for_cellid(self):
      ''' Clears a private variable containing cell ids and their locations
         ...
         .. code-block: python
             ...
             for vlsvReader in vlsvReaders:
                # Print something from the file on the screen
                logger.info vlsvReader.read_variable("B", cellids=2) # Stores info into a private variable
                # Upon reading from vlsvReader a private variable that contains info on cells that have blocks has been saved -- now clear it to save memory
