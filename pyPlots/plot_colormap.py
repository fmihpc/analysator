...
def plot_colormap(filename=None,
                  ...
                  fluxcoords=None,fluxlevels=None,fluxfilevlsv=None,
                  fsaved=None,
                  Earth=None,
                  highres=None,
    ...
    :kword fluxcoords:  Plot fluxfunction contour lines from these coordinates (e.g. from x-points)
    :kword fluxlevels:  Plot fluxfunction contour lines at these isolevels
    :kword fluxfilevlsv: File to sniff out the fluxfile extents
...
#    if pass_vars:
    ...
    if fluxfile:
        ...
        [xsizef, ysizef, zsizef] = pt.vlsvfile.VlsvReader(fluxfilevlsv).get_spatial_mesh_size()
        simextf = pt.vlsvfile.VlsvReader(fluxfilevlsv).get_spatial_mesh_extent()/6371e3
        print(simextf, [xsizef, zsizef])
        [XmeshXYf,YmeshXYf] = scipy.meshgrid(np.linspace(simextf[0],simextf[3],num=xsizef+1),np.linspace(simextf[2],simextf[5],num=zsizef+1))
        # The grid generated by meshgrid has all four corners for each cell.
        # We mask using only the centre values.
        # Calculate offsets for cell-centre coordinates
        XmeshCentresf = XmeshXYf[:-1,:-1] + 0.5*(XmeshXYf[0,1]-XmeshXYf[0,0])
        YmeshCentresf = YmeshXYf[:-1,:-1] + 0.5*(YmeshXYf[1,0]-YmeshXYf[0,0])

        #flux_function = np.fromfile(fluxfile,dtype='double').reshape(sizes[1],sizes[0])
        flux_function = np.fromfile(fluxfile,dtype='double').reshape(zsizef,xsizef)
        Xvec = XmeshCentresf[0,:]
        Yvec = YmeshCentresf[:,0]
        print(Xvec,Yvec)
        #print(flux_function.shape, np.sum(np.all([Yvec >= boxcoords[2], Yvec <= boxcoords[3]], axis = 0)))
        #flux_function = flux_function[np.all([Yvec >= boxcoords[2], Yvec <= boxcoords[3]], axis = 0),:]
        #print(flux_function.shape)
        #flux_function = flux_function[:, np.all([Xvec >= boxcoords[0], Xvec <= boxcoords[1]], axis = 0)]
        #print(flux_function.shape, XmeshCentres.shape)
        if False:
        ...
        if fluxlevels is None:
            flux_levels = np.linspace(-10,10,fluxlines*60)
        elif fluxlevels is not None:
            flux_levels = list(fluxlevels)
            flux_levels.sort()
            print(flux_levels)
        else:
            # too large for interpolation in the whole BCH domain..
            interpolator = scipy.interpolate.interp2d(XmeshCentres,YmeshCentres,flux_function)
            flux_levels = interpolator(fluxcoords[:,0], fluxcoords[0,2])
        fluxcont = ax1.contour(XmeshCentresf,YmeshCentresf,flux_function,flux_levels,colors='k',linestyles='solid',linewidths=0.5*fluxthick,zorder=2, nchunk=100)
