...
def inplane(inputarray):
    ...
    if PLANE=='XY':
        ...
        logger.info("Error defining plane!")
        return -1
    return inputarray
...
def numvecdotdelvec(inputarray1, inputarray2):
    ...
    if inputarray1.shape!=inputarray2.shape:
        logger.info("Error: Input array shapes don't match!",inputarray1.shape,inputarray2.shape)
        return -1
...
def expr_Diff(pass_maps, requestvariables=False):
    ...
    if (map0.shape != map1.shape):
        logger.info("Error with diff: incompatible map shapes! ",map0.shape,map1.shape)
        sys.exit(-1)
    if (map0.dtype in ['uint8','uint16','uint32','uint64']):
        logger.info("Diff: Converting from unsigned to signed integers")
...
def expr_Slippage(pass_maps, requestvariables=False):
    ...
    if type(pass_maps) is list:
        logger.info("expr_Slippage expected a single timestep, but got multiple. Exiting.")
        quit()
...
def expr_EcrossB(pass_maps, requestvariables=False):
    ...
    # Verify that time averaging wasn't used
    if type(pass_maps) is list:
        logger.info("expr_EcrossB expected a single timestep, but got multiple. Exiting.")
        quit()
...
def expr_betatron(pass_maps, requestvariables=False):
    ...
    if type(pass_maps) is not list:
        ...
        logger.info("expr_betatron expected a list of timesteps to average from, but got a single timestep. Exiting.")
        quit()
...
def expr_diamagnetic(pass_maps, requestvariables=False):
    ...
    if type(pass_maps) is not list:
        # Not a list of time steps, calculating this value does not make sense.
        logger.info("expr_diamagnetic expected a list of timesteps to average from, but got a single timestep. Exiting.")
        quit()
...
def expr_jc(pass_maps, requestvariables=False):
    ...
    # Verify that time averaging wasn't used
    if type(pass_maps) is list:
        logger.info("expr_jc expected a single timestep, but got multiple. Exiting.")
        quit()
...
def expr_jg(pass_maps, requestvariables=False):
    ...
    # Verify that time averaging wasn't used
    if type(pass_maps) is list:
        logger.info("expr_jg expected a single timestep, but got multiple. Exiting.")
        quit()
...
def expr_jgyr(pass_maps, requestvariables=False):
    ...
    # Verify that time averaging wasn't used
    if type(pass_maps) is list:
        logger.info("expr_jgyr expected a single timestep, but got multiple. Exiting.")
        quit()
...
def expr_jring(pass_maps, requestvariables=False):
    ...
    # Verify that time averaging wasn't used
    if type(pass_maps) is list:
        logger.info("expr_jring expected a single timestep, but got multiple. Exiting.")
        quit()
...
def expr_jp(pass_maps, requestvariables=False):
    ...
    if type(pass_maps) is not list:
        # Not a list of time steps, calculating this value does not make sense.
        logger.info("expr_jp expected a list of timesteps to average from, but got a single timestep. Exiting.")
        quit()
...
def expr_jm(pass_maps, requestvariables=False):
    ...
    # Verify that time averaging wasn't used
    if type(pass_maps) is list:
        logger.info("expr_jm expected a single timestep, but got multiple. Exiting.")
        quit()
...
def expr_ja(pass_maps, requestvariables=False):
    ...
    # Verify that time averaging wasn't used
    if type(pass_maps) is list:
        logger.info("expr_ja expected a single timestep, but got multiple. Exiting.")
        quit()
...
def expr_dLstardt(pass_maps, requestvariables=False):
    ...
    if type(pass_maps) is not list:
        # Not a list of time steps, calculating this value does not make sense.
        logger.info("expr_dLstardt expected a list of timesteps to average from, but got a single timestep. Exiting.")
        quit()
...
def expr_electronflow(pass_maps, requestvariables=False):
    ...
    # Verify that time averaging wasn't used
    if type(pass_maps) is list:
        logger.info("expr_electronflow expected a single timestep, but got multiple. Exiting.")
        quit()
    ...
    logger.info("mean jprot",np.mean(np.linalg.norm(jprot,axis=-1)),'mean jele',np.mean(np.linalg.norm(jele,axis=-1)))
    logger.info("min jprot",np.min(np.linalg.norm(jprot,axis=-1)),'min jele',np.min(np.linalg.norm(jele,axis=-1)))
    logger.info("max jprot",np.max(np.linalg.norm(jprot,axis=-1)),'max jele',np.max(np.linalg.norm(jele,axis=-1)))

    logger.info("mean reqjv",np.mean(np.linalg.norm(reqjv,axis=-1)),'mean reqjele',np.mean(np.linalg.norm(reqjele,axis=-1)))
    logger.info("min reqjv",np.min(np.linalg.norm(reqjv,axis=-1)),'min reqjele',np.min(np.linalg.norm(reqjele,axis=-1)))
    logger.info("max reqjv",np.max(np.linalg.norm(reqjv,axis=-1)),'max reqjele',np.max(np.linalg.norm(reqjele,axis=-1)))
...
def cavitons(ax, XmeshXY,YmeshXY, extmaps, requestvariables=False):
    ...
    logger.info("cavitons",cavitons.sum())
...
def expr_electronpressure_isothermal(pass_maps, requestvariables=False):
    ...
    # Verify that time averaging wasn't used
    if type(pass_maps) is list:
        logger.info("expr_electronpressure_isothermal expected a single timestep, but got multiple. Exiting.")
        quit()
    ...
    logger.info("upstream temperature ",T_0)
...
def expr_electronpressure_polytropic(pass_maps, requestvariables=False):
    ...
    # Verify that time averaging wasn't used
    if type(pass_maps) is list:
        logger.info("expr_electronpressure_polytropic expected a single timestep, but got multiple. Exiting.")
        quit()
    ...
    logger.info("upstream pressure ",P_0," upstream density ",n_0)
...
def expr_electronpressure_ratio(pass_maps, requestvariables=False):
    ...
    # Verify that time averaging wasn't used
    if type(pass_maps) is list:
        logger.info("expr_electronpressure_ratio expected a single timestep, but got multiple. Exiting.")
        quit()
...
def expr_fgvgbvol(pass_maps, requestvariables=False):
    ...
    # Verify that time averaging wasn't used
    if type(pass_maps) is list:
        logger.info("expression expected a single timestep, but got multiple. Exiting.")
        quit()
