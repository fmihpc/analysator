...
def set_yticks( figure, yticks ):
   ...
   new_figure = figure
   if yticks <= 1:
      logger.info("BAD YTICKS SET AT SET_YTICKS!")
      return []
   # Get sub axes
...
def plot_variables( x, y, figure=[] ):
   ...
   if x_dim != y_dim:
      if x_dim == y_dim - 1:
         ...
         logger.info("ERROR; BAD X AND Y DIMENSIONS " + str(x_dim) + " " + str(y_dim))
         return []
   else:
...
def plot_multiple_variables( variables_x_list, variables_y_list, figure=[], clean_xticks=False ):
   ...
   if len(variables_x_list) != len(variables_y_list):
      logger.info("BAD VARIABLE LENGTH: " + str(len(variables_x_list)) + " " + str(len(variables_y_list)))
      return []
   if len(variables_y_list) > 18:
      logger.info("TOO MANY VARIABLES: " + str(len(variables_y_list)))
      return []
