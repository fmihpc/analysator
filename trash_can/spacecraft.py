...
class SpaceCraft(object):
   ...
   def __init__(self, time, dt, filenames, variables, dt_global=sys.maxint, coordinates=np.zeros(3), cellid=-1):
      ...
      self.__xcells = self.__vlsvfile.read_parameter(name="xcells_ini")
      # Do the same for y
      self.__ymax = self.__vlsvfile.read_parameter(name="ymax")
      self.__ymin = self.__vlsvfile.read_parameter(name="ymin")
      self.__ycells = self.__vlsvfile.read_parameter(name="ycells_ini")
      # And for z
      self.__zmax = self.__vlsvfile.read_parameter(name="zmax")
      self.__zmin = self.__vlsvfile.read_parameter(name="zmin")
      self.__zcells = self.__vlsvfile.read_parameter(name="zcells_ini")
      ...
      self.__values = dict()
      for varName in np.atleast_1d(self.__variables):
   ...
   def increase_time(self, updatevariables=False, t_step=-1):
      ...
      # Check if the time index has changed:
      while True:
         if currentTime + dt >= (self.__time_index + 1)*self.__dt_global:
      ...
      #    self.__time_index = self.__time_index + 1
      #    if self.__time_index >= len(self.__filenames):
   ...
   def record_next_value(self):
      ...
      if dt_to_next_frame < smallest_dt:
         return self.increase_time(updatevariables=True, t_step=dt_to_next_frame)
      else:
   ...
   def get_values_array(self):
      '''Returns the values recorded by space craft (python dictionary)
      '''
      return self.__values

   #O: THIS DID NOT TURN OUT TO BE VERY USEFUL MIGHT WANT TO MAKE A ZOOMED VERSION OF THIS
   #def plot_route(self):
   #   #pl.ion()
   ...
   #   #pl.ylim((ymin, ymax))
   #   #pl.show()
   #   inputFileNameAndCoordinates={}
   #   fileName = self.__filenames[0][0:(len(self.__filenames[0])-5)] + ".silo"
   #   coordinates=[]
   #   i = 0
   #   while i < len(self.__values["xcoordinates"]):
   #      coordinates.append([self.__values["xcoordinates"][i], self.__values["ycoordinates"][i], self.__values["zcoordinates"][i]])
   #      i = i + 20
